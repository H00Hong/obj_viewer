...
from mywxwidgets.dataview import DataViewModel, DataRow, dv
from .NdArrayWXShow2D import MainWin


FONT0 = (14, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL, 
         wx.FONTWEIGHT_NORMAL, False, 'JetBrains Mono')
...
class _ShowWin(wx.Frame):
    def __init__(self, parent, val=['name', 'type', 'size', 'value', 'obj']):
        ...
        self.lab = wx.TextCtrl(self, value=val[3], style=wx.TE_MULTILINE|wx.TE_READONLY|wx.TE_PROCESS_ENTER)
...
class ViewerFrame(wx.Frame):
    def __init__(self, parent=None, dr=(), col=[('name', 210), ('type', 280), ('size',), ('value',)]):
        wx.Frame.__init__(self, parent=parent, title='对象查看器',size=(1200, 700))

        self.dvc = dv.DataViewCtrl(self, style=dv.DV_SINGLE | dv.DV_VERT_RULES
                                   | dv.DV_HORIZ_RULES | dv.DV_ROW_LINES)

        self.model = DataViewModel(dr)
        self.dvc.AssociateModel(self.model)
        for i, v in enumerate(col):
            self.dvc.AppendTextColumn(v[0], i, dv.DATAVIEW_CELL_ACTIVATABLE, 
...
def get_drs(lists=[('魔术方法和属性', []), ('方法', []), ('属性', [])], header=None):
    ...
    for i, v in enumerate(lists):
        ...
        dat.append(DataRow((i + ds,), [v[0], '', str(len(v[1])), '', None])) # type: ignore
...
class ObjectViewerFrame(ViewerFrame):
    ...
    def __init__(self, parent=None, obj=None, name:str='', 
                 col=[('name', 210), ('type', 280), ('size', ), ('value', )]) -> None:
    ...
    def _set(self, _obj:object, name:str=''):
        ...
        return get_drs([('魔术方法和属性', dirs_magic), ('方法', dirs_method), ('属性', dirs_var)], 
                       set_dr_list(_obj, name))


class DictViewerFrame(ObjectViewerFrame):
    def __init__(self, parent=None, obj={}) -> None:
        ObjectViewerFrame.__init__(self, parent, obj, 
                               col=[('name', 210), ('value_type', 280), ('size', ), ('value', )])
...
class SeqViewerFrame(ObjectViewerFrame):
    def __init__(self, parent=None, obj=()):
        ObjectViewerFrame.__init__(self, parent, obj, 
                               col=[('index',), ('type', 180), ('size',), ('value',)])
    
    def _set(self, _obj:Sequence, name:str=''):
        return [DataRow((i,), set_dr_list(v, str(i))) for i, v in enumerate(_obj)]
        


class VariableViewerFrame(ViewerFrame):
    def __init__(self, args:Dict[str, object]):
        ViewerFrame.__init__(self, None, self._set(args),
                               [('name', 210), ('type', 280), ('size', ), ('value', )])
        self.SetTitle('变量查看器')
